...
nPMprec=[0,2,2,2,2,2,1];fPMavg=[0,0,0,0,0,0,0];sCWD="";sFNprefix="HN_MELI_NUOML1";sFn2sv="";sFn2fw="";dt2sv=datetime.now()
def pmsHead():
    sh="DateTime"
...
while True:
    try:
        ...
        fn=fc=fw=sCWD+sFNprefix+"_";fn+=dt.strftime("%Y%m%d%H")+".txt";fc+="PMsCur.txt";fw+=dt.strftime("%Y%m%d")+"flowin.txt" #"0000.txt"
        if sFn2sv!="" and os.path.exists(fn)==False:
            ...
            with open(sFn2fw,"a") as f:f.write(ss)   #+"\n" 2026se2we@LvL 
            for i in range(np):nPMcnt[i]=0;fPMsum[i]=0 #2026se2we@LvL
        sFn2sv=fn;sFn2fw=fw;dt2sv=dt;ss="";sc=""
        for i in range(np):
