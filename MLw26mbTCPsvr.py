...
def updRegs():#print("BackgroundThread started.modifying inRegs..")
    while True:#generate some randomized SensorReadings temp=random.randint(0,999);#humi=random.randint(45,65)
        ...
        try:
            with open("/home/pi/HN_MELI_NUOML1/HN_MELI_NUOML1_PMsCur.txt","rb",buffering=0) as f: #"r") as f:#Line=f.readline();
                bs=f.readline();Line=bs.decode("utf-8");m3h=int(Line.split("\t")[1])
                MBtcpSvr.data_bank.set_input_registers(0,[m3h]);time.sleep(3)
        except Exception:time.sleep(3);continue #if __name__ == "__main__":
try:
